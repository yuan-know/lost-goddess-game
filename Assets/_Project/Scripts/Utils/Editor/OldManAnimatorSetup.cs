// ============================================================================
//  OldManAnimatorSetup.cs —— 老年形态 Animator Controller 脚手架生成(Editor)
//
//  用途:一键生成老年形态的动画状态机,与 PlayerController 的接口对齐:
//        · 参数:isWalking (bool)   —— PlayerController.SetMoving 会 SetBool 它
//        · 状态:Idle(默认) / Walk
//        · 过渡:Idle → Walk (isWalking == true)
//                Walk → Idle (isWalking == false)
//        · 附带生成两个占位 AnimationClip(Old_Idle / Old_Walk)并挂进对应状态,
//          Walk 设为 loop。美术在 Animation 窗口对这两个 clip 摆关键帧即可,
//          无需再手动连状态机。
//
//  产物目录:Assets/_Project/Art/Animations/Old/
//        · OldMan.controller
//        · Old_Idle.anim(空,待摆帧)
//        · Old_Walk.anim(空,loop,待摆帧)
//
//  接线(手工,见指南):把老年角色 GameObject 的 Animator.controller 指向 OldMan.controller,
...
{
    public static class OldManAnimatorSetup
    {
        const string Dir = "Assets/_Project/Art/Animations/Old";
        const string ControllerPath = Dir + "/OldMan.controller";
        const string IdleClipPath = Dir + "/Old_Idle.anim";
        const string WalkClipPath = Dir + "/Old_Walk.anim";
        const string WalkParam = "isWalking"; // 必须与 PlayerController.walkParam 一致

        [MenuItem("失落的女神/生成老年Animator脚手架")]
        public static void Generate()
        {
            ...
            Debug.Log("[OldManAnimatorSetup] 已生成 " + ControllerPath +
                      "\n参数 isWalking + 状态 Idle/Walk + 双向过渡。占位 clip:Old_Idle / Old_Walk(待摆帧)。");
            EditorUtility.DisplayDialog("完成",
                "老年 Animator 脚手架已生成:\n" + ControllerPath +
                "\n\n· 参数 isWalking(bool),PlayerController 会自动驱动它\n" +
                "· 状态 Idle(默认)/ Walk,已连好双向过渡\n" +
                "· 占位 clip Old_Idle / Old_Walk —— 用 Animation 窗口对它们摆关键帧\n\n" +
                "接线:老年角色的 Animator.controller 指向 OldMan.controller,\n" +
