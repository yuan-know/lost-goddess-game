...
num("PSB 已在项目:Assets/_Project/Art/Characters/characters_3forms.psb(青/中/老三组,每组12部位层+1完整立绘层)。")
callout("为什么是 .psb 不是 .psd:Unity 的 2D PSD Importer 默认只接管 .psb,普通 .psd 走 TextureImporter"
        "(当普通贴图,无法分层/绑骨骼)。.psb 与 .psd 内部结构一致,改扩展名即无损切换。本项目角色分层图统一用 .psb。", "warn")
...
para("成功后,在 Project 窗口点开 characters_3forms.psb 左边的小三角,应能看到子 Sprite:"
...
num("Project 里选中 characters_3forms.psb(选整个 psd,不是子 Sprite)。")
...
num("characters_3forms.psb 从 Project 拖进 Hierarchy —— 因开了 Generate GO Hierarchy,"
